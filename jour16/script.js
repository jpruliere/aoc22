...
Object.values(valves).forEach((valve) => {
  ...
  // optimal positioning : number of valves / sum of distances from other relevant valves
  valve.positioning = Object.keys(valves).length / Object.entries(distances).reduce((a, [v, s]) => v === start ? a : a + s, 0)
})

const relevantValves = Object.fromEntries(Object.entries(valves).filter(([, { name, flow }]) => flow > 0 || name === start))

const computePositioning = (valves, visited) => {
  const from = [...visited].pop()
  Object.values(valves).forEach(valve => {
    const relevantDistances = Object.entries(valve.distances).filter(([name]) => !visited.includes(name))
...
const findPaths = (distances, path, flow = 0, minutes = 30) => {
  ...
  for (const destination in distances) {
    ...
    const nextDistancesAsEntries = Object.entries(relevantValves[destination].distances).filter(([name]) => !path.includes(name))
